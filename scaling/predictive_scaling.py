...
def get_kube_config():
    ...
    configuration.ssl_ca_cert = None # In prod, write cluster['certificateAuthority']['data'] to a temp file
...
def is_spike_imminent(schedule: list, lead_time_minutes=30) -> Tuple[bool, List[dict]]:
    ...
    if spike_imminent:
        logger.info(f"Found {len(approaching_games)} games starting within {lead_time_minutes} minutes")
...
def trigger_karpenter_scale_up(apps_v1, core_v1):
    ...
    template = client.V1PodTemplateSpec(
        ...
        spec=client.V1PodSpec(containers=[container], priority_class_name="low-priority")
    )
    ...
    try:
        ...
        if e.status == 409: # Already exists
            apps_v1.patch_namespaced_deployment(name=DEPLOYMENT_NAME, namespace="default", body={"spec": {"replicas": 10}})
...
def trigger_karpenter_scale_down(apps_v1):
    ...
    try:
        apps_v1.patch_namespaced_deployment(name=DEPLOYMENT_NAME, namespace="default", body={"spec": {"replicas": 0}})
    except client.exceptions.ApiException as e:
        if e.status == 404:
