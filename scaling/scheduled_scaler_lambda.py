...
def release_lock(lock_id: str, lambda_request_id: str) -> bool:
    ...
    try:
        ...
        logger.warning(f"Failed to release lock (will expire in {LOCK_TTL_SECONDS}s): {e}")
        return False
...
def lambda_handler(event: Dict, context) -> Dict:
    ...
    emit_metric('ScalingInvocations', 1, dimensions=[
        {'Name': 'DryRun', 'Value': str(DRY_RUN_MODE)}
    ])
    ...
    if not lock_acquired:
        ...
        emit_metric('ScalingSkipped', 1, 'Count', [{'Name': 'Reason', 'Value': 'LockContention'}])
        return {
            'statusCode': 200,
            'body': json.dumps({
                'status': 'skipped',
    ...
    try:
        ...
        spike_imminent, games_approaching = is_spike_imminent(schedule, lead_time_minutes=LEAD_TIME_MINUTES)

        if DRY_RUN_MODE:
            logger.info(f"DRY RUN: Would check for spike with lead time {LEAD_TIME_MINUTES}m")
            ...
            ] if games_approaching else ["Would scale down (no imminent games)"]

        if spike_imminent:
            logger.info(f"Spike imminent! {len(games_approaching)} games starting within {LEAD_TIME_MINUTES} minutes")
            ...
            if not DRY_RUN_MODE:
                try:
                    ...
                    if not conn_test['connected']:
                        raise EKSAuthError(f"Cluster connection failed: {conn_test['error']}")
                    ...
                    logger.info(f"Successfully triggered scale-up for cluster {EKS_CLUSTER_NAME}")

                except EKSAuthError as e:
                    logger.error(f"EKS authentication failed: {e}")
                    result['errors'].append(f"EKS auth: {str(e)}")
                    result['status'] = 'failed'
                    emit_metric('ScalingErrors', 1, dimensions=[{'Name': 'ErrorType', 'Value': 'EKSAuth'}])

                except Exception as e:
                    logger.error(f"Scale-up failed: {e}")
                    result['errors'].append(f"Scale-up: {str(e)}")
                    result['status'] = 'failed'
                    emit_metric('ScalingErrors', 1, dimensions=[{'Name': 'ErrorType', 'Value': 'ScaleUp'}])
            else:
        ...
        emit_metric('ScalingErrors', 1, dimensions=[{'Name': 'ErrorType', 'Value': 'Unexpected'}])
        ...
        logger.info(f"Scaling check completed in {duration:.2f}s with status: {result['status']}")
