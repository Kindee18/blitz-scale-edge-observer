...
def process_log_events(log_events):
    ...
    for event in log_events:
        ...
        if (
            ...
            or "critical" in message.lower()
            or "gameplay_event" in message.lower()
        ):
...
def lambda_handler(event, context):
    ...
    logger.info(
        f"Processed {len(log_events)} events from {log_group}. Dropped: {dropped_count}, Retained: {len(filtered_logs)}"
    )
    ...
    if filtered_logs:
        ...
        try:
            s3_client.put_object(
                Bucket=S3_BUCKET, Key=object_key, Body=payload.encode("utf-8")
            )
            logger.info(
                f"Successfully wrote {len(filtered_logs)} logs to s3://{S3_BUCKET}/{object_key}"
            )
